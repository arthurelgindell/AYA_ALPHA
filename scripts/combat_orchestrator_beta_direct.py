#!/usr/bin/env python3
"""
GLADIATOR Combat Orchestrator - BETA Direct Execution
Runs directly on BETA for optimal performance (no SSH tunnel overhead)
"""
...
# Configuration (BETA Direct Execution)
LM_STUDIO_ALPHA = "http://alpha.local:1234"            # Blue Team (ALPHA via network)
LM_STUDIO_BETA = "http://localhost:1234"               # Red Team (BETA local)
BLUE_MODEL = "foundation-sec-8b-instruct-int8"
RED_MODEL = "llama-3.3-70b-instruct"

# Data paths (BETA filesystem)
ARMED_EXPLOITS = Path("/Volumes/DATA/GLADIATOR/armed_exploits")
TRAINING_OUTPUT = Path("/Volumes/DATA/GLADIATOR/combat_training")
...
Generate WORKING attack code (not theoretical):"""
        ...
        try:
            response = requests.post(
                ...
                timeout=180  # 3 minutes for 70B model local execution
            )
...
Generate ACTIONABLE defense response:"""
        ...
        try:
            response = requests.post(
                ...
                timeout=180  # 3 minutes for network access
            )
...
def run_combat_session(session_id, rounds=10):
    ...
    print(f"\n{'='*80}")
    print(f"🥊 GLADIATOR COMBAT SESSION {session_id}")
    print(f"{'='*80}")
    print(f"Red Team: {RED_MODEL} (BETA - LOCAL)")
    print(f"Blue Team: {BLUE_MODEL} (ALPHA - NETWORK)")
    ...
    for i, exploit in enumerate(exploits, 1):
        print(f"\n--- Round {i}/{rounds} ---")
        print(f"Exploit: {exploit.get('cve', 'N/A')} | Persona: {exploit.get('persona', 'N/A')}")
        
        # Red Team attacks (LOCAL - FAST)
        ...
        # Blue Team defends (NETWORK - via alpha.local)
        ...
        print(f"{status} | Detection Score: {outcome['detection_score']}/6")
    ...
    print(f"📊 Combat session complete!")
...
║                   BETA Direct Execution Mode                     ║
╚══════════════════════════════════════════════════════════════════╝

Configuration:
  Sessions: {sessions}
  Rounds per session: {rounds_per_session}
  Red Team Model: {RED_MODEL} (LOCAL - BETA)
  Blue Team Model: {BLUE_MODEL} (NETWORK - ALPHA)
