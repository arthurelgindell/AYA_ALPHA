#!/bin/bash
# Deploy Grafana Dashboard System on ALPHA node
# Usage: ./deploy_alpha.sh

set -e

echo "========================================="
echo "AYA Grafana Dashboard - ALPHA Deployment"
echo "========================================="
echo

# Verify we're on ALPHA
HOSTNAME=$(hostname)
if [[ ! "$HOSTNAME" =~ alpha ]]; then
    echo "⚠️  Warning: This script is intended for ALPHA node"
...
docker-compose -f ../docker-compose-alpha.yml down 2>/dev/null || true
echo
...
nohup python3 /Users/arthurdell/AYA/services/grafana/exporters/aya_metrics_exporter.py > /tmp/aya_metrics_exporter.log 2>&1 &
echo "✅ AYA metrics exporter started (port 9200)"

# Start Tailscale exporter
pkill -f tailscale_exporter.py 2>/dev/null || true
nohup python3 /Users/arthurdell/AYA/services/grafana/exporters/tailscale_exporter.py > /tmp/tailscale_exporter.log 2>&1 &
...
echo "========================================="
echo
echo "Access Grafana at: http://alpha.tail5f2bae.ts.net:3000"
echo "Username: arthur"
echo "Password: AyaGrafana2025!"
echo
echo "Logs:"
echo "  Docker:         docker-compose -f /Users/arthurdell/AYA/services/grafana/docker-compose-alpha.yml logs -f"
